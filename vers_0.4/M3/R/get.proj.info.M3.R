...
# 
# NOTES:
#   1. The R package sf depends on the PROJ cartographic projections library
#      (http://trac.osgeo.org/proj/).  The format of the string must therefore
#      be in a style acceptable to PROJ.  (See
#.     https://proj.org/en/9.4/usage/quickstart.html).
#   2. See information about the meaning of Models-3 projection
#      arguments at
...
get.proj.info.M3 <- function(file, earth.radius=6370000){
  ...
  # kilometers.  If so, it would probably be within these bounds.
  ...
  # one of those listed above.
    ...
    nc <- nc_close(nc)
    stop(
      paste("Grid type ", grid.type, " cannot be handled by this function.",
            sep=""))
  }
  # ##############################################
