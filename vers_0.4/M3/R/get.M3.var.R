...
#
#.  2024-05-31 (JLS): Updated code formatting.
# ###########################################################
get.M3.var <- function(file, var, lcol, ucol, lrow, urow,
  ...
  if (missing(var)){
    nc <- nc_close(nc)
    stop( paste("Specify name or number of variable to read.  Variable names are: ",
                paste(all.varnames, collapse=", "), sep="") )
  }
  ...
  if (is.numeric(var)){
    ...
    if ( !(trunc(var) == var) ){
      nc <- nc_close(nc)
      stop( paste("Parameter var must be a whole number between 1-",
                  length(all.varnames), sep="") )
    }
    ...
    if ( (var < 1) || (var > length(all.varnames)) ){
      nc <- nc_close(nc)
      stop( paste("File ", file, " contains variables numbered 1-",
                  length(all.varnames), sep="") )
    }
  }
  ...
  if ( !is.numeric(var) && !is.character(var) ){
    nc <- nc_close(nc)
    stop("Parameter var must give the name or number of variable to be read." )
  }
  # ##########################
  ...
  if (ucol < lcol){
    nc <- nc_close(nc)
    stop(paste("Upper column limit, ", ucol,
               ", is less than lower column limit, ", lcol, sep=""))
  }
  ...
  if (urow < lrow){
    nc <- nc_close(nc)
    stop(paste("Upper row limit, ", urow, ", is less than lower column limit, ",
               lrow, sep=""))
  }
  ...
  if (ulay < llay){
    nc <- nc_close(nc)
    stop( paste("Upper layer limit, ", ulay,
                ", is less than lower layer limit, ", llay, sep="") )
  }


  # Check to make sure row, column, and layer limits are positive numbers.
  if ( (lcol <= 0) && (ucol <= 0) && (lrow <= 0) && (urow <= 0) && (llay <= 0)
       && (ulay <= 0)){
    nc <- nc_close(nc)
  ...
  if (tstep.incr != 0){
    ...
    if (udatetime < ldatetime){
      nc <- nc_close(nc)
      stop(paste("Upper date-time bound, ", udatetime,
                 ", is before lower date-time bound, ", ldatetime, sep=""))
    }


    # Find the dates in the sequence which fall in the specified range.
    which.datetime <- which( (ldatetime <= datetime.seq) &
                               (datetime.seq <= udatetime) )
  ...
  extracted.data <- ncvar_get( nc, varid=var,
                               start=c(start.col, start.row, start.lay,
                                       start.datetime),
                               count=c(count.col, count.row, count.lay,
                                       count.datetime) )
  ...
  if (!identical(all.x.coord$units, all.y.coord$units))
    stop(
      paste("Error: Units for x-coordinates and y-coordinates differ. For x, units are ",
            all.x.coord$units, "; for y, ", all.y.coord$units, ".", sep="") )
  else
