...
#
# ASSUMES:
#   1. Availability of R packages ncdf4 and sf.
...
#     rather than relying on project() and SpatialPoints object from deprecated
#     "rgdal" and "sp" packages. Updated code formatting.
# ###########################################################
project.M3.1.to.M3.2 <- function(x, y, from.file, to.file, units, ...){
  ...
  # project.lonlat.to.M3.  Warn user that this function is
  # being called.
  if ( substring(from.proj.string, first=7, last=13)=="longlat" ){
    warning(paste('The specified "from" projection in file,', from.file,
      ' is longitude/latitude.  Using function project.lonlat.to.M3...',
      sep=""))
  ...
  # project.M3.to.lonlat(). Warn user that this function is
  # being called.
  if ( substring(to.proj.string, first=7, last=13)=="longlat" ){
    warning(paste('The specified "to" projection in file,', to.file,
      ' is longitude/latitude.  Using function project.M3.to.lonlat.',
      sep=""))
    return(project.M3.to.lonlat(x=x, y=y, file=from.file, units=units, ...))
  }

  
  # (3) If neither projection is long/lat, proceed with the remainder of
  # this function.
  ...
  # ##########
  # Project locations from original projection to new projection.
  to.coords <- sf_project(from.coords, from=from.proj.string, to=to.proj.string)
  
  # Return the coordinates, adjusting units appropriately 
  if (units=="km")
    return(list(coords=to.coords/1000, units=units))
  else
    return(list(coords=to.coords, units=units))
  # ##########
